...
COCO adds creator metrics and account-based reviews to creator profiles, plus a local post-velocity radar on X.
...
- Supports an optional user-supplied TikHub follower profile with an explicit possible-charge notice.
...
- `storage`: stores the signed login session, panel and X-radar preferences, optional user-supplied TikHub key and up to 100 cached public creator results locally.
...
- `https://api.tikhub.io/*`: retrieves an optional creator follower profile only after the user configures their own TikHub key and requests the operation.
...
1. Install version 1.3.2 and open a supported creator profile.
2. The COCO card appears near the profile avatar and immediately shows locally read public metrics when available.
3. The Overview, Followers and Reviews tabs separate the main record, public profile context and signed-in user reviews.
...
7. TikHub is optional. With no TikHub key, the extension continues to operate from page data and the exact personal-library lookup.
