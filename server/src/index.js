...
function privacyPage() {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>COCO Creator Intel Privacy</title><style>body{max-width:760px;margin:48px auto;padding:0 22px;font:16px/1.65 system-ui;color:#24262b}h1,h2{line-height:1.2}code{background:#f2f3f5;padding:2px 5px;border-radius:4px}</style></head><body><h1>COCO Creator Intel Privacy Policy</h1><p>Last updated: September 3, 2026.</p><p>The extension reads public creator profile identifiers and public metrics already loaded on supported TikTok, Instagram, YouTube and X pages. It sends only the public platform and handle to the COCO Creator Intel service for an exact lookup and review retrieval.</p><h2>Google sign-in</h2><p>Any Google account may sign in. The service processes the verified Google subject identifier, email, display name and profile image to issue a session, attribute user-submitted reviews and enforce access. It requests only <code>openid</code>, <code>email</code> and <code>profile</code>; it does not request or read Gmail, Drive, Calendar, contacts or passwords.</p><h2>Creator reviews</h2><p>Signed-in users may voluntarily submit a rating label and review text for a creator. Reviews are stored by the COCO service with the author's verified email and display name. Other signed-in users see the display name, rating, review text and date, but not the author's email address. Authors may delete their own reviews; the service owner may remove any review for moderation.</p><h2>Private owner data</h2><p>Historical prices and past collaboration records are returned only when the verified email exactly matches the configured owner account. Other accounts receive public creator fields only. This check is enforced by the server.</p><h2>Local X velocity radar</h2><p>On X, the extension may locally calculate public-post average exposure velocity and a current-tab leaderboard from public views and timestamps already loaded by the page. It does not call the X API or upload post content, post metrics, velocity results or leaderboard data.</p><h2>Optional TikHub use</h2><p>A user may supply their own TikHub API key. It remains in local extension storage and is sent only to TikHub after the user requests enhanced public profile information or an Instagram audience-country analysis. Audience analysis requests recent public Reels, public liker identifiers/usernames and public account-location fields, then locally caches the aggregate analysis and a bounded identifier-to-country lookup cache so repeated analyses do not repay for the same public result. One analysis is capped at approximately 313 TikHub requests.</p><h2>Storage and deletion</h2><p>The extension stores its session, user preferences, optional TikHub key, up to 100 cached public profile/audience results and a bounded public country-result cache in Chrome local storage. The service stores voluntarily submitted creator reviews and the minimum account identity needed for authorship and moderation. Removing the extension or clearing its storage deletes local data; users may delete their own reviews or email the address below for a broader deletion request.</p><h2>Limited Use</h2><p>The use of information received from Google APIs will adhere to the Chrome Web Store User Data Policy, including the Limited Use requirements.</p><h2>Contact</h2><p><a href="mailto:ahuocicicicici@gmail.com">ahuocicicicici@gmail.com</a></p></body></html>`;
}
